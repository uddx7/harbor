      ...
      >
        ...
        {isSpread ? (
          ...
          >
            ...
            {saved ? t("Saved") : t("Bookmark page {n}", { n: manga.pageIndex + 1 })}
          </button>
        )}
                  ...
                  <div key={bm.id} className="flex items-center gap-1 rounded-2xl px-1 active:bg-raised/40">
                    ...
                    >
                      ...
                      <span className="truncate text-[12.5px] text-ink-subtle">
                        {missing ? t("Not in this source") : `${bm.chapterLabel} · ${t("Page {n}", { n: bm.page })}`}
                        <span className="text-ink-subtle/70"> · {relativeTime(bm.createdAt)}</span>
                      </span>
                    </button>
                    <button
                      type="button"
